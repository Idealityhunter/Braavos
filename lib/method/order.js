Meteor.methods({
  // 尚未支付, 关闭订单
  "order.close": (orderId, consumerId, commodityName, reason, order) => {
    // TODO 乐观锁,检测当前状态
    ...
    // Deprecated: 服务端需要发送交易消息的提示
    //if (res && Meteor.isServer) Meteor.call('msg.send.order', consumerId, orderId, commodityName, 'close', reason);
    if (res && Meteor.isServer) Meteor.call('viae.marketplace.onCancelOrder', order, reason);
    return res;
  },
  ...
  "order.commit": (orderId, consumerId, commodityName, order) => {
    ...
    if (resRefundDeny && Meteor.isServer){
      ...
      //Meteor.call('msg.send.order', consumerId, orderId, commodityName, 'commit');
      Meteor.call('viae.marketplace.onRefundDeny', order);
      Meteor.call('viae.marketplace.onCommitOrder', order);
    };
  ...
  // 拒绝退款
  "order.refundDeny": (orderId, consumerId, commodityName, memo, order) => {
    // TODO 乐观锁,检测当前状态
    ...
    // 通过viae发送消息
    if (res && Meteor.isServer) Meteor.call('viae.marketplace.onRefundDeny', order, memo);
    return res;
  }
});